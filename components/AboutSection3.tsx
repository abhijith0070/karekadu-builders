...
export default function AboutSection3() {
  ...
  return (
    <section className="relative bg-white overflow-hidden" ref={heroRef}>
      ...
      <div className="relative min-h-[85vh] flex items-center pt-16">
      ...
      <div className="bg-white py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Team Accordion */}
          <div className='group flex max-sm:flex-col justify-center gap-2 sm:gap-3 md:gap-4 w-full mx-auto max-w-7xl'>
            {teamMembers.map((item, i) => {
              return (
                <article 
                  key={item.id}
                  className='group/article relative w-full rounded-xl overflow-hidden sm:group-hover:[&:not(:hover)]:w-[20%] sm:group-focus-within:[&:not(:focus-within):not(:hover)]:w-[20%] transition-all duration-300 ease-[cubic-bezier(.5,.85,.25,1.15)] before:absolute before:inset-x-0 before:bottom-0 before:h-1/3 before:bg-gradient-to-t before:from-black/50 before:transition-opacity sm:before:opacity-0 sm:hover:before:opacity-100 max-sm:before:opacity-100 focus-within:before:opacity-100 after:opacity-0 sm:group-hover:[&:not(:hover)]:after:opacity-100 sm:group-focus-within:[&:not(:focus-within):not(:hover)]:after:opacity-100 after:absolute after:inset-0 after:bg-white/30 after:backdrop-blur after:rounded-lg after:transition-all focus-within:ring focus-within:ring-blue-300'
                >
                  <div
                    className='absolute inset-0 text-white z-10 p-4 sm:p-5 md:p-6 flex flex-col justify-end'
                  >
                    <h3 className='text-lg sm:text-xl md:text-2xl font-medium sm:whitespace-nowrap sm:truncate max-sm:opacity-100 sm:opacity-0 group-hover/article:opacity-100 group-focus-within/article:opacity-100 sm:translate-y-2 group-hover/article:translate-y-0 group-focus-within/article:translate-y-0 transition duration-200 ease-[cubic-bezier(.5,.85,.25,1.8)] group-hover/article:delay-300 group-focus-within/article:delay-300'>
                      {item.title}
                    </h3>
                    <span className='text-xl sm:text-2xl md:text-3xl font-medium sm:whitespace-nowrap sm:truncate max-sm:opacity-100 sm:opacity-0 group-hover/article:opacity-100 group-focus-within/article:opacity-100 sm:translate-y-2 group-hover/article:translate-y-0 group-focus-within/article:translate-y-0 transition duration-200 ease-[cubic-bezier(.5,.85,.25,1.8)] group-hover/article:delay-500 group-focus-within/article:delay-500'>
                      {item.description}
                    </span>
                  </div>
                  <Image
                    className='object-cover h-[400px] sm:h-[500px] md:h-[600px] lg:h-[700px] w-full'
                    src={item.url}
                    width={960}
                    height={480}
                    alt={item.title}
